"use client";
// Exclusão de clipe (admin-only via RLS) com confirmação em dois passos —
...
import { excluirClipe, type EstadoAcao } from "@/app/(app)/artista/[slug]/clipes/actions";

const ESTADO_INICIAL: EstadoAcao = { status: "idle" };

export function ExcluirClipeButton({ id, caminho }: { id: string; caminho: string }) {
  const [confirmando, setConfirmando] = useState(false);
  const [estado, formAction, pendente] = useActionState(excluirClipe, ESTADO_INICIAL);
