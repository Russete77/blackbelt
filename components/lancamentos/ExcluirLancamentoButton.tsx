"use client";
// Exclusão de lançamento (admin-only via RLS) com confirmação em dois passos
...
import { excluirLancamento, type EstadoAcao } from "@/app/(app)/artista/[slug]/lancamentos/actions";

const ESTADO_INICIAL: EstadoAcao = { status: "idle" };

export function ExcluirLancamentoButton({ id, caminho }: { id: string; caminho: string }) {
  const [confirmando, setConfirmando] = useState(false);
  const [estado, formAction, pendente] = useActionState(excluirLancamento, ESTADO_INICIAL);
